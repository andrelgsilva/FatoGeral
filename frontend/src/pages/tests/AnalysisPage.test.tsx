...
describe('Home Page — Formulário de análise', () => {
  ...
  it('shouldDisableSubmitButtonDuringRequest', async () => {
    ...
    await userEvent.type(
      screen.getByPlaceholderText(/cole aqui o texto/i),
      'texto suspeito'
    );
    ...
    expect(
      screen.getByRole('button', { name: /analisando/i })
    ).toBeDisabled();
  });
  ...
  it('shouldDisplayErrorToastWhenApiCallFails', async () => {
    ...
    expect(
      await screen.findByRole('button', { name: /analisar/i })
    ).toBeInTheDocument();

    expect(toast.error).toHaveBeenCalledWith(
      'Erro ao enviar para análise. Tente novamente.'
    );
  });
